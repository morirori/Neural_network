...
class SOMNetwork(AbstractNetwork):
    ...
    def train(self, data,label,repetitions):
    ...
    def set_winner_neuron_label(self, label):
        ...
        # print()
#        print ("Winner output vector = {} and layer_id {} and id {} label {}".format(winner.output_vector, winner.layer_id, winner.id, label))
...
#        print("reset")
        for i in range(1,len(self.layers_list)):
            for j in range(len(self.layers_list[i].neuron_vector)):
                self.layers_list[i].neuron_vector[j].reset_color_marker()
